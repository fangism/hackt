...
BEGIN {
	...
	print "// This file is automatically generated by wrap_verilog_modules_to_hacprsim.awk.";
	print "";
}
...
function emit_from_prsim(name) {
	return "$from_prsim({prsim_name, \"." name \
		"\"}, {verilog_name, \"." name "\"});";
}

function emit_to_prsim(name) {
	return "$to_prsim({verilog_name, \"." name \
		"\"}, {prsim_name, \"." name "\"});";
}

# emit wrapper function
# no parameters, variables are only local
function write_out_wrapper(tmp, u, l, n) {
...
for (p in ports) {
	...
	for ( ; l<=u; ++l) {
		...
		print "\t" (reverse ? emit_to_prsim(p) : emit_from_prsim(p));
	}
	}
	if (match(type, "out")) {
		good = 1;
	...
	for ( ; l<=u; ++l) {
		...
		print "\t" (reverse ? emit_from_prsim(p) : emit_to_prsim(p));
	}
	}
	if (!good) {
