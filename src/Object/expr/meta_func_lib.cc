/**
	...
	$Id: meta_func_lib.cc,v 1.1.2.1 2010/09/15 00:57:54 fang Exp $
 */
...
typedef	std::map<string, meta_mapped_func_ptr_type>	
					meta_func_map_type;
...
int
register_meta_function(const string&, const meta_function_ptr_type);
#endif

//=============================================================================
meta_function_registrar::meta_function_registrar(
		const string& fn, const meta_function_ptr_type fp) {
	if (register_meta_function(fn, fp)) {
		THROW_EXIT;
	}
}
...
meta_function_registrar::meta_function_registrar(
	...
	if (register_meta_function(fn, meta_function_ptr_type(fp))) {
		THROW_EXIT;
	}
}
...
register_meta_function(const string& fn, const meta_function_ptr_type fp) {
	STACKTRACE_VERBOSE;
	NEVER_NULL(fp);
	meta_mapped_func_ptr_type& mf(meta_function_map[fn]);
	if (!mf) {
	if (ack_loaded_functions) {
		// confirmation:
		cout << "loaded function: `" << fn << "'." << endl;
	}
		mf = meta_mapped_func_ptr_type(fp);
	} else {
...
lookup_meta_function(const std::string& fn) {
	...
	if (i == meta_function_map.end()) {
		...
		return meta_function_ptr_type(NULL);
	} else {
		return i->second;
	}
}
...
META_FUNCTION_LOAD_DEFAULT("sprintf", strcat)
