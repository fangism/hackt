/**
	...
	Class method definitions for semantic expression.  
 	$Id: art_object_value_reference.tcc,v 1.1.2.1 2005/03/10 07:18:55 fang Exp $
 */
...
VALUE_REFERENCE_CLASS::hash_string(void) const {
	return parent_type::hash_string();
}
...
VALUE_REFERENCE_CLASS::has_static_constant_dimensions(void) const {
	return parent_type::has_static_constant_dimensions();
}
...
VALUE_REFERENCE_CLASS::static_constant_dimensions(void) const {
	return parent_type::static_constant_dimensions();
}
...
pint_instance_reference::pint_instance_reference() :
		param_instance_reference(), pint_expr(), value_collection_ref(NULL) {
}
...
pint_instance_reference::initialize(const init_arg_type& i) {
	return value_collection_ref->initialize(i);
}
...
pint_instance_reference::must_be_equivalent_pint(const pint_expr& i) const {
	...
	if (ir) {
		...
		// This will not be true if the language allows nested 
		// templates, so beware in the distant future!
		const size_t lpos = value_collection_ref->is_template_formal();
		const size_t rpos = ir->value_collection_ref->is_template_formal();
		if (lpos && rpos && (lpos == rpos)) {
...
pint_instance_reference::unroll_resolve_value(
	...
	if (array_indices) {
		...
		const never_ptr<pint_scalar>
			scalar_inst(value_collection_ref.is_a<pint_scalar>());
		NEVER_NULL(scalar_inst);
		return scalar_inst->lookup_value(i);
	}
}
...
pint_instance_reference::unroll_resolve(const unroll_context& c) const {
	typedef	count_ptr<const_param>		return_type;
	STACKTRACE("value_collection_ref::unroll_resolve()");
	if (value_collection_ref->get_dimensions()) {
		// dimension resolution should depend on current 
		// state of instance collection, not static analysis
		// from compile phase.
		...
		do {
			// populate the collection with values
			// lookup_value returns true on success, false on error
			if (!value_collection_ref->lookup_value(*coll_iter, key_gen).good) {
				cerr << "ERROR: looking up index " <<
					key_gen << " of pint collection " <<
					value_collection_ref->get_qualified_name() <<
					"." << endl;
		...
		value_type _val;
		const never_ptr<pint_scalar>
			ps(value_collection_ref.is_a<pint_scalar>());
		INVARIANT(ps);
		const bad_bool valid(ps->lookup_value(_val));
		if (valid.bad) {
			cerr << "ERROR: in unroll_resolve-ing "
...
pint_instance_reference::assign(const list<value_type>& l) const {
	return value_collection_ref->unroll_assign(l);
}
#endif
...
pint_instance_reference::load_object(const persistent_object_manager& m, 
	...
	NEVER_NULL(value_collection_ref);
	m.load_object_once(
		const_cast<pint_instance_collection*>(&*value_collection_ref));
	load_object_base(m, f);
}
...
pint_instance_reference::assigner::operator() (const bad_bool b, 
	...
	if (ranges.empty()) {
		...
		// is scalar assignment, but may be indexed
		const never_ptr<pint_scalar> 
			scalar_inst(p.value_collection_ref.is_a<pint_scalar>());
		if (scalar_inst) {
			return bad_bool(scalar_inst->assign(vals.front())) || b;
		}
	}
