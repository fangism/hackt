...
static
node_index_type
parse_node_to_local_index(const string& s, const entity::module& m) {
	return dispatch_parser(ptr_fun(parser::parse_node_to_local_index),
		s, m, false).index;
}
...
if (a.size() != 4) {
	...
	++ai;
	// indices are local
	const node_index_type srcind = parse_node_to_local_index(*ai, s.get_module());
	if (!srcind) {
		cerr << "Error finding node: " << *ai << endl;
		return Command::BADARG;
	}
	++ai;
	const node_index_type dstind = parse_node_to_local_index(*ai, s.get_module());
	if (!dstind) {
		cerr << "Error finding node: " << *ai << endl;
		return Command::BADARG;
	}
...
if (asz < 4 || asz > 5) {
	...
	const node_index_type srcind = parse_node_to_local_index(rnode, s.get_module());
	if (!srcind) {
	...
	const node_index_type dstind = parse_node_to_local_index(tnode, s.get_module());
	if (!dstind) {
	...
	if (asz == 5) {
		++ai;
		predind = parse_node_to_local_index(*ai, s.get_module());
		if (!predind) {
