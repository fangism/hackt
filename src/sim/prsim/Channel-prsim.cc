...
channel::reset(vector<env_event_type>& events) {
	...
	if (is_sinking()) {
		events.push_back(env_event_type(ack_signal, 
			(get_ack_init() ? LOGIC_HIGH : LOGIC_LOW)));
	}
...
if (stopped()) {
...
#endif
	{
		...
		for ( ; key[0] < bundles(); ++key[0]) {
			...
			case LOGIC_LOW:
				if (get_ack_active() ^ (av == LOGIC_HIGH)) {
				...
				break;
			case LOGIC_HIGH:
				if (get_ack_active() ^ (av == LOGIC_LOW)) {
					s.__node_why_not(o, di, 
						limit, !why_not, 
						why_not, verbose, u, v);
				}
...
if (ni == ack_signal) {
	...
	case LOGIC_HIGH:
		...
		break;
	default:
		new_events.push_back(env_event_type(ack_signal, LOGIC_OTHER));
		break;
	}
	}
#endif
...
#endif
	if (x_counter) {
		events.push_back(env_event_type(ack_signal, LOGIC_OTHER));
	} else if (!counter_state) {
