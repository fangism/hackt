/**
	...
	$Id: placement_engine.cc,v 1.3 2011/05/05 06:50:46 fang Exp $
 */
 ...
 */
ostream&
placement_engine::emit_fig(ostream& o) const {
	const save_precision s(o, 3);
	const save_flags f(o, o.flags());
	o << std::fixed;
	// xfig preamble -- I don't remember what all these mean
	document d;		// just take default values
	d.comment = "generated by ipple:emit-fig";
	d.emit(o);
	o << "# bounding box" << endl;
	polyline p;
	p.depth = 200;
	const position_type& ll(opt.lower_corner);
	const position_type& ur(opt.upper_corner);
	const real_type r = opt.xfig_scale *default_resolution;
	p.define_box(point(ll[0]*r, ll[1]*r), point(ur[0]*r, ur[1]*r));
	p.style_val = util::fig::xfig::XFIG_LINE_DASHED;
	p.emit(o);
	space.emit_fig(o, opt);
	return o;
}
